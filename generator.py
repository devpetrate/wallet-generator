...
def generate(count: int = None, chain : str = "ETH", bip = None):
    if count == None:
        logger.info(f"You have not specified the number of generations, five will be made.")
        count = 5
    date = {'mnemonic': [],
            'private': [],
            'address': []
    ...
    if os.name == "posix":
        ...
        path = f"wallets.xlsx"
        logger.success(f"GENERATION COMPLETE, check file {path} in your directory")
    logger.success(f"GENERATION COMPLETE, check file {path}")
